...
class FinalEnhancedAttentionTracker:
    """
    Final optimized attention tracker with enhanced MediaPipe detection
    """
    ...
    def __init__(self, camera_index: int = 0, frame_width: int = 640, 
        ...
        print("📱 Enhanced MediaPipe Detection (No YOLO)")
    ...
    def detect_hand_near_face(self, hands, face_landmarks):
        """Enhanced hand near face detection"""
        if not hands or not face_landmarks:
            return False

        # Use multiple facial landmarks for better detection
        ...
        for hand in hands:
            for lm in hand.landmark:
                ...
                distance = np.sqrt((lm.x - face_center_x)**2 + (lm.y - face_center_y)**2)
                if distance < 0.12:  # More sensitive threshold
                    return True
        return False

    def analyze_hand_gesture(self, hands, face_landmarks):
        """Analyze hand gesture to detect phone usage patterns"""
        if not hands or not face_landmarks:
            return {"phone_gesture": False, "confidence": 0.0, "gesture_type": "none"}
        ...
        for hand in hands:
            ...
            index_extension = np.sqrt((index_tip.x - index_pip.x)**2 + (index_tip.y - index_pip.y)**2)
            middle_extension = np.sqrt((middle_tip.x - middle_pip.x)**2 + (middle_tip.y - middle_pip.y)**2)
            
            # Phone detection criteria
            is_pinching = thumb_index_dist < 0.05  # Thumb and index close
            fingers_extended = index_extension > 0.03 and middle_extension > 0.03
            hand_near_face = self.detect_hand_near_face([hand], face_landmarks)
            
            if hand_near_face:
                if is_pinching and fingers_extended:
                    phone_gesture = True
                    confidence = 0.9
    ...
    def process_frame(self, frame: np.ndarray) -> Dict[str, Any]:
        ...
        if hands_results.multi_hand_landmarks:
            ...
            if results["face_detected"]:
                ...
                # Analyze hand gesture for phone usage
                gesture_analysis = self.analyze_hand_gesture(
                    hands_results.multi_hand_landmarks, 
                    results["face_landmarks"]
                )
                results["phone_gesture"] = gesture_analysis["phone_gesture"]
                results["gesture_confidence"] = gesture_analysis["confidence"]
                results["gesture_type"] = gesture_analysis["gesture_type"]
                
                # Phone near face if gesture indicates phone usage
                results["phone_near_face"] = gesture_analysis["phone_gesture"]
    ...
    def draw_gesture_debug_panel(self, frame, metrics):
        ...
        debug_text = [
            ...
            f"Confidence: {metrics['gesture_confidence']:.2f}",
            f"Pattern: {metrics['gesture_type']}"
        ]
    ...
    def run(self):
        ...
        print("📱 Enhanced MediaPipe Detection")
        ...
        while True:
            ...
            print(json.dumps({
                ...
                "gesture_type": results["gesture_type"],
                "gesture_confidence": results["gesture_confidence"],
                "fps": results["fps"]
            }, indent=None))
